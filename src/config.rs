...
impl nat_cell {
    ...
    ", cell = self, portStart = port_start, portEnd = port_end, remoteIP = match remote_ip(remote_domain) {
                        Some(s) => s,
            ...
            nat_cell::SINGLE { local_port, remote_port, remote_domain } =>
                {
                    ...
                    ", cell = self, localPort = local_port, remotePort = remote_port, remoteIP = match remote_ip(remote_domain) {
                        Some(s) => s,
                        None => return "".to_string(),
                    }, localIP = match local_ip() {
                        Some(s) => s,
