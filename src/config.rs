    ...
    pub fn build(&self) -> String {
        ...
        match &self {
            NatCell::Range {
                ...
                format!("# {cell:?}\n\
                    {tcpPrefix}add rule ip nat DIY_PREROUTING tcp dport {portStart}-{portEnd} counter dnat to {dstIp}:{portStart}-{portEnd}\n\
                    {udpPrefix}add rule ip nat DIY_PREROUTING udp dport {portStart}-{portEnd} counter dnat to {dstIp}:{portStart}-{portEnd}\n\
                    {tcpPrefix}add rule ip nat DIY_POSTROUTING ip daddr {dstIp} tcp dport {portStart}-{portEnd} counter snat to {localIP}\n\
                    {udpPrefix}add rule ip nat DIY_POSTROUTING ip daddr {dstIp} udp dport {portStart}-{portEnd} counter snat to {localIP}\n\n\
            ...
            NatCell::Single {
                ...
                if dst_domain == "localhost" || dst_domain == "127.0.0.1" {
                    // 重定向到本机
                    format!("# {cell:?}\n\
                        {tcpPrefix}add rule ip nat DIY_PREROUTING tcp dport {localPort} redirect to :{remotePort}\n\
                        {udpPrefix}add rule ip nat DIY_PREROUTING udp dport {localPort} redirect to :{remotePort}\n\n\
                        ", cell = self, localPort = src_port, remotePort = dst_port, tcpPrefix = protocol.tcp_prefix(), udpPrefix = protocol.udp_prefix())
                } else {
                    // 转发到其他机器
                    format!("# {cell:?}\n\
                        {tcpPrefix}add rule ip nat DIY_PREROUTING tcp dport {localPort} counter dnat to {dstIp}:{dstPort}\n\
                        {udpPrefix}add rule ip nat DIY_PREROUTING udp dport {localPort} counter dnat to {dstIp}:{dstPort}\n\
                        {tcpPrefix}add rule ip nat DIY_POSTROUTING ip daddr {dstIp} tcp dport {dstPort} counter snat to {localIP}\n\
                        {udpPrefix}add rule ip nat DIY_POSTROUTING ip daddr {dstIp} udp dport {dstPort} counter snat to {localIP}\n\n\
