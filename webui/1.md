下面是包含所有必要修改后的完整代码，包括 `bcryptTool.js`、`server.js`、`public/index.html` 和 `public/admin.html` 文件。
...
<body>
    <div class="container">
        ...
        <label>
            转发模式:
            <select id="forwardMode">
                <option value="">SINGLE</option>
                <option value="RANGE">RANGE</option>
            </select>
            <span class="note">空值表示单一转发，请根据实际需求选择。</span>
        </label>
...
1. **证书路径**: 确保在 `server.js` 中替换 `/root/nftables-nat-rust-webui/ssl/private-key.pem` 和 `/root/nftables-nat-rust-webui/ssl/certificate.pem` 为您的实际 SSL 证书和私钥路径。
   
2. **读取权限**: 确保 Node.js 进程可以读取 `/etc/nat.conf` 和 `passwd.md` 文件。
   
3. **密码文件**: 确保 `passwd.md` 的格式为 `用户名:哈希密码`，例如 `admin:$2b$10$gY9KnYXxJ.PqybUkf0z2y.VD2LZX1X5LfKoJu9zW0PzW.q34654eO`。
   
4. **安装依赖**: 确保已用以下命令安装必要的 npm 包：
   ```bash
...
有关如何将 Node.js 应用设置为开机自启动，请遵循以下步骤：

1. **创建服务文件**: 在 `/etc/systemd/system/nftables-nat-rust-webui.service` 中创建一个服务文件。
