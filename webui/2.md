好的，以下是去掉 "转发模式" 的完整代码版本，保留了 "规则类型" 的功能，并相应地更新了文件。
...
1. **证书路径**: 请确保在 `server.js` 中替换 `/root/nftables-nat-rust-webui/ssl/private-key.pem` 和 `/root/nftables-nat-rust-webui/ssl/certificate.pem` 为您的实际 SSL 证书和私钥路径。
