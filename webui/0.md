为了满足您的需求，我已经为项目添加了以下功能：

1. 当规则类型选择为 `SINGLE` 时，起始端口显示为本机端口，结束端口显示为目标端口。
2. 为当前规则添加了编辑功能。

以下是您所需的完整代码，已针对这两个功能进行了相应的修改。
...
对 `server.js` 文件进行了相应的修改，以支持规则的编辑功能和端口自动填充。
...
在这个文件中添加了编辑功能以及根据选择的规则类型自动填充端口信息的功能。
...
- 确保在 `server.js` 中的证书路径是正确的。
- 确保 Node.js 进程对 `/etc/nat.conf` 和 `passwd.md` 具备相应的读取和写入权限。
- 代码中的测试和使用需在合适的环境中进行，确保符合您的开发与部署要求。
